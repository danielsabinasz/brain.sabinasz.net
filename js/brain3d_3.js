...
var brain3d = (function() {
    ...
    var flash = false;
    var default_opacity_2d = 0.5;
    var default_opacity_3d = 0.0;
    var mni_image = "t1";

    var default_camera_distance = 190;
    ...
    function createViews() {
        for (var view in views) {
            ...
            scenes[view].background = new THREE.Color(0xffffff);
            ...
            // Build renderer
            renderers[view] = new THREE.WebGLRenderer( { antialias: true, alpha: false } );
            renderers[view].setClearColor( 0xffffff );
    ...
    function animate() {
        ...
        /*var angle = Math.round((180 + Math.atan2(cameras["3d"].position.z, cameras["3d"].position.x)*180.0 / Math.PI)%360);
        ...
        if (record) {
                $.post("https://brain.sabinasz.net/record.php?" + angle, {"filename": angle, "data": dataURL}, function() { console.log("success " + angle); }).fail(function() { console.log("error " + angle); });
                //console.log(angle + " record");
        } else console.log(angle + " do not record");*/

	var progress = (num % 600)/600.0;
	var x_lower;
	if (progress <= 0.5) x_lower = talairach.x_min + 2*progress*(talairach.x_max-talairach.x_min);
	    else x_lower = talairach.x_max - 2*(progress-0.5)*(talairach.x_max-talairach.x_min);
	talairach.setRanges([x_lower/10.0, talairach.x_max/10.0], [talairach.y_min, talairach.y_max], [talairach.z_min, talairach.z_max]);
	console.log(progress, x_lower);
        num++;
	stats.update();
    }


	            var camera_pos = perspectives["left_lateral"]["pos"].clone().normalize().multiplyScalar(default_camera_distance);
            cameras["3d"].position.set(camera_pos.x, camera_pos.y, camera_pos.z);
            cameras["3d"].lookAt(new THREE.Vector3(0, 0, 0));
            cameras_brain_regions["3d"].position.set(camera_pos.x, camera_pos.y, camera_pos.z);
            cameras_brain_regions["3d"].lookAt(new THREE.Vector3(0, 0, 0));
	...
	setShowGrayMatter: function(show) {
        ...
        setPerspective: function(perspective_name) {
            var camera_pos = perspectives[perspective_name]["pos"].clone().normalize().multiplyScalar(default_camera_distance);
            cameras["3d"].position.set(camera_pos.x, camera_pos.y, camera_pos.z);
            cameras["3d"].lookAt(new THREE.Vector3(0, 0, 0));
            cameras_brain_regions["3d"].position.set(camera_pos.x, camera_pos.y, camera_pos.z);
            cameras_brain_regions["3d"].lookAt(new THREE.Vector3(0, 0, 0));
